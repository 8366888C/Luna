...
import type { portfolioCategoryType, portfolioConfig } from "@/lib/types";
...
export function FilterPortfolio({ items }: FilterPortfolioProps) {
  const filter = useFilter(items as portfolioConfig[], "portfolio");
  ...
  return (
    <div className="flex flex-col gap-3 animation">
              ...
              <div className="flex flex-wrap gap-1.5">
                {allCategories.map((cat) => (
                  <button
                    ...
                    type="button"
                    onClick={() =>
                      handleCategoryChange(cat as portfolioCategoryType)
                    }
                    className={cn(
      ...
      {filteredAndSortedItems.length === 0 ? (
        <p className="paragraph text-center mt-12">No projects found.</p>
      ) : (
