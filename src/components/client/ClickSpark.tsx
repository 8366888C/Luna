...
// ... (Interfaces Spark and ClickSparkProps remain the same)
...
const ClickSpark: React.FC<ClickSparkProps> = ({
  ...
  useEffect(() => {
    ...
    // Watch for shadcn theme toggles
  ...
  useEffect(() => {
    ...
    const draw = (timestamp: number) => {
      ...
      sparksRef.current = sparksRef.current.filter((spark: Spark) => {
        ...
        ctx.strokeStyle = resolvedColor; // Use the dynamic state color
        ctx.lineWidth = 2;
        ctx.lineCap = "round";
        ctx.beginPath();
        ctx.moveTo(x1, y1);
